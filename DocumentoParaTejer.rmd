...
modulo_ocupados <- rename(modulo_ocupados, area = AREA,
                          ...
                          cotiza_pension = P6920, exp_cotiza = P6960)
...
```{r}
edad_summary <- c(
  min(datos_cruce$edad, na.rm = TRUE),
  quantile(datos_cruce$edad, probs = 0.25, na.rm = TRUE),
  median(datos_cruce$edad, na.rm = TRUE),
  mean(datos_cruce$edad, na.rm = TRUE),
  quantile(datos_cruce$edad, probs = 0.75, na.rm = TRUE),
  max(datos_cruce$edad, na.rm = TRUE),
  sum(is.na(datos_cruce$edad))
)

edad_tabla <- matrix(edad_summary, nrow = 1)
colnames(edad_tabla) <- c("Mínimo", "Cuartil 1", "Mediana", "Promedio", "Cuartil 3", "Máximo", "NA's")

kable(
  edad_tabla,
  caption = "Resumen de la Variable Edad",
  escape = F,
  booktab = T,
  align = "c")
```
...
```{r, fig.cap = "Histograma de Edad", warning = FALSE}
# Histograma
ggplot(datos_cruce, aes(x = edad)) +
  geom_histogram(fill = "skyblue", color = "black", bins = 30) +
  labs(title = NULL,
       x = "Edad",
       y = "Frecuencia") +
...
```{r, fig.cap = "Diagrama de Caja y Bigotes de Edad", warning = FALSE}
# Diagrama de caja y bigotes
ggplot(datos_cruce, aes(x = "", y = edad)) +
  geom_boxplot(fill = "skyblue", color = "black") +
  labs(title = NULL,
       x = NULL,  # Elimina la etiqueta del eje x
       y = "Edad") +
